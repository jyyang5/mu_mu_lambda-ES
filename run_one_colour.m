


%fun_fitness_sigmaStar(ita,n,scatterColour,typeDot,LINE_OR_NOT)
%Input
%   ita:                  % ita = sigma_ep_star /sigma_star
%   n:                    dim of data
%   scatterColour:        r  Red, g  Green, b   Blue, c   Cyan, m  Magenta,y  Yellow, k   Black,w  White
%   typeDot:              type of dot (+  Plus sign, o  Circle, *  Asterisk, x    Cross)
%   LINE_OR_NOT:          if just scatter = 0
%Return
%   scatter or scatter+curve

figure(4);
% green
ita = 4;
%fun_fitness_sigmaStar(4,100000,'g','.',1);        
fun_fitness_sigmaStar(ita,10,'g','x',0);        
fun_fitness_sigmaStar(ita,100,'g','o',0);

disp('green done');


% red
ita = 1;
fun_fitness_sigmaStar(ita,10,'r','x',0);        
fun_fitness_sigmaStar(ita,100,'r','o',0);

disp('red done');
% blue
ita = 0.25;
fun_fitness_sigmaStar(ita,10,'b','x',0);        
fun_fitness_sigmaStar(ita,100,'b','o',0);

disp('blue done');


% black
ita = 0;
fun_fitness_sigmaStar(ita,10,'k','x',0);        
fun_fitness_sigmaStar(ita,100,'k','o',0);

disp('black done');

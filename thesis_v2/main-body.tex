...
\end{align}
To obtain the opt. expected fitness gain $\eta_{opt}$ and its corresponding opt. normalized step size $\sigma^*_{opt}$ over a fixed noise-to-signal ratio $\vartheta$, assuming $\vartheta$ and $\sigma$ we take derivative of equation (\ref{eqn:eta_surrogate}) over $\sigma^*$ and get the following 

...
In generation $(g+1)$, we denote the parental population $X^{(g+1)} = \{x_1,x_2,...,x_\mu\}$ and offspring generated $Y^{(g+1)} = \{y_1,y_2,...,y_{\lambda} \}$, where $x_i,y_j \in \mathbb{R}^N$ for $i=1,...,\mu$ and $j=1,...,\lambda$. In offspring generation, a standard normally distributed mutation vector $z_i \in \mathbb{R}^N $ is generated and added to the centroid with a step size parameter $\sigma \in \mathbb{R}$ and we have 
\begin{align}

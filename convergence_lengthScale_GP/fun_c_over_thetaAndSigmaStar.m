% [2D-PLOT for different lambda and theta] Add theta_array GP length scale  
...
%            original
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val = fun_c_over_thetaAndSigmaStar(f,name,NUM_OF_RUNS,sigma_star_array,lambda_array,theta_array,TRAINING_FACTOR,strategy_name)
%Input:
...
c_array = zeros(LAMBDA_LENGTH,SIGMA_LENGTH,THETA_LENGTH,NUM_OF_RUNS,1);                 % convergence rate
s_array = zeros(LAMBDA_LENGTH,SIGMA_LENGTH,THETA_LENGTH,NUM_OF_RUNS,1);                 % success rate
t_array = zeros(LAMBDA_LENGTH,SIGMA_LENGTH,THETA_LENGTH,NUM_OF_RUNS,1);                 % # of iterations 
T_array = zeros(LAMBDA_LENGTH,SIGMA_LENGTH,THETA_LENGTH,NUM_OF_RUNS,1);                 % # of objective function calls
f_x_matrix = zeros(LAMBDA_LENGTH, SIGMA_LENGTH,THETA_LENGTH,NUM_OF_RUNS,10000);          % store all fx
T_med = zeros(LAMBDA_LENGTH,SIGMA_LENGTH,THETA_LENGTH);
t_med = zeros(LAMBDA_LENGTH,SIGMA_LENGTH,THETA_LENGTH);
f_x_med = zeros(LAMBDA_LENGTH,SIGMA_LENGTH,THETA_LENGTH,10000);
...
if name == 6
    ...
    fprintf(fileID,d);
    fprintf(fileID,'%15s \t','(mu/mu,lambda)theta'); 
elseif name == 7
...
for i = 1:1:LAMBDA_LENGTH
    ...
    for j = 1:1:THETA_LENGTH
        theta_temp = theta_array(j);
        d1 =sprintf('(%d/%d,%d) %d \t,',mu,mu,lambda_temp,theta_temp);
        fprintf(fileID,d1); 
        % plot convergece rate of sigma_temp over sigma_star
        figure(name);
        hold on;
        d =sprintf('(%d/%d,%d),%d',mu,mu,lambda_temp,theta_temp);
        temp_c = transpose(squeeze(c_med(i,j,:)));
        temp = temp_c(temp_c~=0);
        len = length(temp);
        scatter(sigma_star_array(1:len),temp,'DisplayName',d); hold on;
        plot(sigma_star_array,temp_c,'DisplayName',d);hold on;
        % write convergence rate for each sigma_star 
        for k = 1:1:SIGMA_LENGTH
            d1 =sprintf('%.4f  \t ',c_med(i,j,k));
            fprintf(fileID,d1); 
        end
        for k = 1:1:SIGMA_LENGTH
            d1 =sprintf('%.4f  \t ',c_med(i,j,k));

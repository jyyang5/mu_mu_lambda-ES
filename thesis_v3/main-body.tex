...
Histograms showing the number of objective function calls needed to solve the test problems within the required accuracy are represented in the first row of Fig. \ref{fig:merged_plot}, the median number of objective function calls required to solve each test problem for $(\mu/\mu,\lambda)$-ES without and with surrogate model assistance is shown in Table \ref{Tab:Test_result_noGP_mml} and Table \ref{Tab:Test_result} respectively; the result for surrogate assisted (1+1)-ES \cite{DBLP:conf/ppsn/KayhaniA18} is also included in Table \ref{Tab:Test_result} for comparison. 


The two speed-ups $\text{speed-up}_{\text{self}}$ and $\text{speed-up}_{\text{model}}$ defined in Section \ref{sec:analysis} Analysis are used. The overall performance of surrogate assisted $(\mu/\mu,\lambda)$-ES does not match that of the surrogate assisted (1+1)-ES, specifically in most of the 10-dimensional test problems used; the $\text{speed-up}_{\text{self}}$s on quadratic sphere for GP-mml-ES with $\lambda=10,20,40$($\mu = \lceil \lambda \rceil $) are between five and six, nine and ten, eighteen and nineteen respectively; whereas the corresponding expected results in Analysis are between nine and ten, fifteen and sixteen, eighteen and nineteen respectively. There is a gap between the expected performance of the surrogate model assisted $(\mu/\mu,\lambda)$-ES after applying the GP surrogate model and CSA. But the gap seems to shrink as the population size $\lambda$ increases. In terms of $\text{speed-up}_{\text{model}}$, the achieved values for the surrogate assisted $(\mu/\mu,\lambda)$-ES are 


The performance of the surrogate model assisted $(\mu/\mu,\lambda)$-ES proposed seems to approach the analytical result obtained in Section \ref{sec:analysis} as the population size 

are between four and six, surrogate assisted $(5/5,20)$-ES between 
...
, as opposed to a value between nine and ten in the analysis in Section \ref{sec:analysis}.

...
%            
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val = fun_multi_run_change_success_rate_FUNCALLS(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE,PROB_RATE_array,TP_S_array,FIGURE_NUM,subplot_ROW,subplot_COL,str_cell_SIGMA_STAR)
%Input:
%   fname:          an index 
%                       1 for linear
%                       2 for quadratic 
...
for q = 1:1:length(PROB_RATE_array)
    ...
    for i = 1:NUM_OF_RUNS
        x0 = randn(n,1);
        sigma0 = 1;
        a = bestSoFar_fourProb_GP_twoLevel(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,PROB_RATE,TP_S_array);
%          = re_bestSoFar_fourProb_GP_SIGMA_STAR(fname,x0,PROB_RATE,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE);
    ...
    d = sprintf('(%d/%d,%d)-ES,S=%.2f,S_2=%.2f',mu,mu,lambda,PROB_RATE,TP_S_array);
...
% legend('show');
    ...
    xlabel('S','FontSize',15); 
    fig_name = sprintf('twoLevel_merged_%d.fig',lambda);
    saveas(gcf,fig_name); 

...
PROB_RATE_array = 0.3:0.1:0.6;
subplot_ROW = 5;
subplot_COL = 5;
str_cell_PROB_RATE = cellstr(num2str(PROB_RATE_array'));

TP_S_ratio = 1/2;
T_matrix = zeros(length(PROB_RATE_array),length(lambda_array),5);             % dim = [sigmaStar, lambda, fname]
...
for i=1:1:length(lambda_array)
	...
	FIGURE_NUM = i;
    for fname = 1:1:5
        temp = fun_multi_run_change_success_rate_FUNCALLS(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE,PROB_RATE_array,TP_S_ratio,FIGURE_NUM,subplot_ROW,subplot_COL,str_cell_PROB_RATE);         

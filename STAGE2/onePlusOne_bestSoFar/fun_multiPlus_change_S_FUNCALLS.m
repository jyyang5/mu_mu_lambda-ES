...
%            
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val = fun_multiPlus_change_S_FUNCALLS(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE,S_arry,FIGURE_NUM,subplot_ROW,subplot_COL,str_cell_SIGMA_STAR)
%Input:
%   fname:          an index 
%                       1 for linear
%                       2 for quadratic 
...
for q = 1:1:length(PROB_RATE_array)
    ...
    for i = 1:NUM_OF_RUNS
        x0 = randn(n,1);
        sigma0 = 1;
        a = onePlusOne_fourProb_GP_bestSoFar(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,S);
...
%     T_range = [T_range_1 T_range_2];
    ...
    d = sprintf('(%d/%d,%d)-ES,S=%.2f',mu,mu,lambda,S);
    ...
    xlabel('C3','FontSize',15); 
    fig_name = sprintf('merged%d_S.fig',lambda);
    saveas(gcf,fig_name); 

% [Refactored] training (1+1)-ES + after bestSoFar (one objFunCall/iteration)
% step size adaptation: success rate 
...
function val = onePlusOne_fourProb_GP_bestSoFar(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,S)
% initialization
% fname:              an index 
%                       1 for linear
%                       2 for quadratic 
...
while((T < NUM_OF_ITERATIONS) && f_centroid > 10^(-8))
    ...
    % (mu/mu, lambda)-ES use GP estiate 
        ...
        % To calculate four probs 
            fep_y_array(t) = gp(xTrain(:,T-TRAINING_SIZE+1:T), fTrain(T-TRAINING_SIZE+1:T), centroid_temp, f_centroid, theta);
            f_y_array(t) = f(centroid_temp);
            f_x_array(t) = f_centroid;
        T = T + 1;
        xTrain(:, T) = centroid_temp;                
        fTrain(T) = f_centroid_temp;
        
        if f_centroid_temp < f_centroid 
            % Centroid is updated in each generation 
            centroid = centroid_temp;
            f_centroid = f_centroid_temp;
    ...
    % Get four probs 
    %%%%%%%%%%%%%%%%%%%%%%%%  
    range_parent = t_start+1:t-2;
    range_offspring = t_start+2:t-1;
    ...
    val = {t,centroid,f_centroid,sigma_array, T, fcentroid_array,convergence_rate,error_array,sigma_star_array,success_rate,delta_array,FOUR_COUNT,1};

...
%            
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val = fun_multi_run_change_success_rate_FUNCALLS(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE,C1_array,C2,C3,FIGURE_NUM,subplot_ROW,subplot_COL,str_cell_SIGMA_STAR)
%Input:
%   fname:          an index 
%                       1 for linear
%                       2 for quadratic 
...
for q = 1:1:length(PROB_RATE_array)
    ...
    for i = 1:NUM_OF_RUNS
        x0 = randn(n,1);
        sigma0 = 1;
        a = bestSoFar_fourProb_GP_dirk(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,C1,C2,C3);
...
% legend('show');
    ...
    xlabel('C1','FontSize',15); 
    fig_name = sprintf('variant_merged%d_%.2f,_%.2f.fig',lambda,C2,C3);
    saveas(gcf,fig_name); 

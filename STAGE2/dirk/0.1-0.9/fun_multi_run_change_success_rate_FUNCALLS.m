...
for q = 1:1:length(PROB_RATE_array)
    ...
    for i = 1:NUM_OF_RUNS
        x0 = randn(n,1);
        sigma0 = 1;
        a = bestSoFar_fourProb_GP_dirk(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,PROB_RATE,TP_S_array);
%          = re_bestSoFar_fourProb_GP_SIGMA_STAR(fname,x0,PROB_RATE,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE);
    ...
    d = sprintf('(%d/%d,%d)-ES,S=%.2f,S_1=%.2f',mu,mu,lambda,PROB_RATE,TP_S_array);

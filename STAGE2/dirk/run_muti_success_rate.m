...
C2 = 0.6;
C3 = 0.6;
C1_array = 0.05:0.05:0.3;
S1 = 0.2;


subplot_ROW = 5;
subplot_COL = 5;
str_cell_PROB_RATE = cellstr(num2str(C1_array'));

T_matrix = zeros(length(C1_array),length(lambda_array),5);             % dim = [sigmaStar, lambda, fname]
...
for i=1:1:length(lambda_array)
	...
	FIGURE_NUM = i;
    for fname = 1:1:5
        temp = fun_multi_run_change_success_rate_FUNCALLS(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE,C1_array,C2,C3,FIGURE_NUM,subplot_ROW,subplot_COL,str_cell_PROB_RATE);         

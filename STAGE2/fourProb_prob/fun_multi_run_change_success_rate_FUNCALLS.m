...
for q = 1:1:length(PROB_RATE_array)
    ...
    for i = 1:NUM_OF_RUNS
        x0 = randn(n,1);
        sigma0 = 1;
        a = re_mml_fourProb_GP_successRate(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,PROB_RATE);
%          = re_bestSoFar_fourProb_GP_SIGMA_STAR(fname,x0,PROB_RATE,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE);
    ...
    d = sprintf('(%d/%d,%d)-ES,S=%.2f',mu,mu,lambda,PROB_RATE);
...
% legend('show');
    ...
    xlabel('S','FontSize',15); 
    fig_name = sprintf('mml_merged_%d.fig',lambda);
    saveas(gcf,fig_name); 

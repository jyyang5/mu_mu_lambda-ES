...
DF = 2; 
C3 = 0.1;
LENGTH_SCALE_array = 4*2*(1:0.5:2.5);
PROB_array = LENGTH_SCALE_array;
...
for i=1:1:length(lambda_array)
	...
	FIGURE_NUM = i;
    for fname = 1:1:5
        temp =  fun_multiPlus_change_lengthScale_FUNCALLS(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE_array,SUCCESS_RATE,DF,C3,FIGURE_NUM,subplot_ROW,subplot_COL,str_cell_SIGMA_STAR);
        T_matrix(:,i,fname) = cell2mat(temp);
    end
end

save('med_T_LS_S=0.4.mat','lambda_array','T_matrix','LENGTH_SCALE','TRAINING_SIZE','NUM_OF_RUNS','SUCCESS_RATE','DF_array','C3','NUM_OF_RUNS');

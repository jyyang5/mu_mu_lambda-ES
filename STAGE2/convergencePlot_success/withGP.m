...
function val = withGP(fname,x0,sigma0,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE)%, OPTIMAL, TARGET_DISTANCE)
% initialization
...
%     GP_error(1:length(t_start+2:T)) = abs(fep_x(t_start+2:T)-f_x(t_start+2:T))./abs(f_x(t_start+1:T-1)-f_x(t_start+2:T));
    ...
    val = {t, x_array(:,T), fx, sigma_funEva_array, T, f_x, convergence_rate,GP_error,sigma_star_array,success_rate,delta_array};

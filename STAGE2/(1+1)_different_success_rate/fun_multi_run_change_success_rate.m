...
%            
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val = fun_multi_run_change_success_rate(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE,SUCCESS_RATE,FIGURE_NUM,subplot_ROW,subplot_COL)
%Input:
%   fname:          an index 
%                       1 for linear
%                       2 for quadratic 
...
for i = 1:NUM_OF_RUNS
    x0 = randn(n,1);
    sigma0 = 1;
    a = onePlusOne_change_success_rate(fname,x0,sigma0,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,SUCCESS_RATE);
...
hold on;
d = sprintf('(1+1)-ES,S=%.2f',SUCCESS_RATE);
...
% plot(1:t_med_noGP, f_x_med_noGP(i,1:t_med_noGP),'DisplayName',d_noGP);hold on; % mml with GP
if(fname==1)
    ylabel(sprintf('Step size (\\lambda)'),'FontSize',15);
end
xlabel('iteration','FontSize',15); 
set(gca, 'YScale', 'log');

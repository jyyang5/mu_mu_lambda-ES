...
function val = mml_GP_CSA_Dirk_hybrid(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE,DECREASE_FACTOR)
% initialization
...
%     /n*dist;                                             % mutation strength/step size(temp)  
    ...
    if(T<=TRAINING_SIZE || t<2)
        s = (1-c)*s+sqrt(mu*c*(2-c))*z;
        sigma = sigma*exp((norm(s)^2-n)/2/D/n);
    elseif(f_centroid > fcentroid_array(t-1))
        sigma = DECREASE_FACTOR*sigma;

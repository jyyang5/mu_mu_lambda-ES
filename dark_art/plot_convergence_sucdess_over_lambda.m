% Each time open a data and plot the summary 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    close all;
    
    if(fname==1||fname==2||fname==3||fname==4||fname==5)
        success_med = median(success_rate_array,2);
        convergence_med = median(convergence_rate_array,2);
        T_med = median(T_array,2);
        subplot(3,1,1)
        hold on;
        plot(lambda_array,success_med);hold on;
        subplot(3,1,2)
        hold on;
        plot(lambda_array,convergence_med);hold on;
        subplot(3,1,3)
        hold on;
        plot(lambda_array,T_med);hold on;
    end
    if(fname==5)
        subplot(3,1,1)
        hold on;
        xlabel('Population size \lambda','fontsize',15);
        ylabel('Success rate rate','fontsize',15);
        legend('linear sphere','quadratic sphere','cubic sphere')
        d =sprintf('Success rate');
        title(d,'fontsize',20);

...
%         h2.BinWidth = 0.2;
        ...
        xLimit = [0 1];
        plot_pdf(convergence_array,T_med,300,3,5,fname,lambda,xNameSprintf,xLimit);
        xNameSprintf = sprintf('success rate');
        xLimit = [0 1];
        plot_pdf(success_med,T_med,300,3,5,fname+5,lambda,xNameSprintf,xLimit);
        xNameSprintf = sprintf('fitGain per iteration');
        xLimit = [-2 2];
        plot_pdf(delta_matrix_temp,T_med,300,3,5,fname+10,lambda,xNameSprintf,xLimit);
        legend('-DynamicLegend'); 
        legend('show');
        saveas(gcf,'success_convergence_delta_plot_v2.fig');  

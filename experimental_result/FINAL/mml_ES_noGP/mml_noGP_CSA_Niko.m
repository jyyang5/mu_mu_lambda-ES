% Using Niko's CSA(paper CMA tutorial)
...
function val = mml_noGP_CSA_Niko(fname,x0,sigma0,lambda,NUM_OF_ITERATIONS,TRAINING_SIZE,LENGTH_SCALE)
% initialization
% fname:              an index 
%                       1 for linear
%                       2 for quadratic 
...
%     end
    ...
    % success rate
    success_rate = sum(fcentroid_array(t_start+1:t-1)>fcentroid_array(t_start+2:t))/length(fcentroid_array(t_start+1:t-1));

...
for fname = 1:1:5
    for i = 1:1:length(lambda_array)
        ...
        temp = fun_multi_run_emergency(fname,NUM_OF_RUNS,lambda,TRAINING_SIZE,LENGTH_SCALE,DECREASE_FACTOR,FIGURE_NUM);
        ...
        if(i > 1)
            emergency_rate_matrix(fname,i,:) = cell2mat(temp(10)); % proportion of emergency [5,1,:] all 0 
        end
    end   
end

save('data_emergency.mat','NUM_OF_RUNS','TRAINING_SIZE','LENGTH_SCALE','DECREASE_FACTOR',...
    't_array','sigma_matrix','T_array','f_x_matrix','convergence_rate_array','GP_error_matrix',...
    'sigma_star_matrix','success_rate_array','delta_matrix','emergency_rate_matrix');

%plot convergence rate and success rate over sigma* and c
function fun_fitness_sigmaStar(ita,n,scatterColour,typeDot,LINE_OR_NOT)
%Input
...
%figure(1)
hold on
scatter(s_start:increment:(s_end), sigma_counvergence_rate_array,typeDot,scatterColour);
...
if n ==10
    d = sprintf('v = %d', ita);
    text((s_start+s_end)/2, median(sigma_counvergence_rate_array), d, 'Color', scatterColour);
end

hold off
ylabel('convergnece rate c');
xlabel('normalized step size \sigma^*');
%set(gca,'xscale','log')

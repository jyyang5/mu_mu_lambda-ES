...
To ensure the model accuracy, an approximate ranking procedure is applied where the true ranking of the complete population is not required. Same offspring generation and selection in $(\mu/\mu,\lambda)$-ES in Section \ref{sssec:def_ES}, $\lambda$ offspring is generated in generation $g$ where the best $\mu$ offspring $y_{1:\mu,\lambda}^{(g)}$ are selected according to the predicted fitness by the model. The ranking of the $\mu$ selected offspring is recorded as $\text{ranking}_{1-\mu;\lambda}^{(g)}$ and the best $n_{\text{init}}$ selected individuals are evaluated using the true objective function and added to update the training set $S$. Then $y_{1:\mu,\lambda}^{(g)}$ are evaluated again by the model that gives a new ranking $\text{ranking}_{1-\mu;\lambda}^{(g \prime)}$, if $\text{ranking}_{1-\mu;\lambda}^{(g)} = \text{ranking}_{1-\mu;\lambda}^{(g \prime)}$, the model is regarded as reliable, otherwise, the next $n_b$ best unevaluated points based on the model prediction is evaluated and added to training set.  

A local weighted regression (LWR) is used as the surrogate model
...
To understand the potential implications of using surrogate models in EAs with varying population size, in this section, we use a simple model for the use of a surrogate model. Specifically, we propose an EA that, in each iteration, a population of $\lambda$ offspring $Y$ are generated and then evaluated by the surrogate model instead of true objective function followed by a intermediate recombination based on the inaccurate surrogate estimate where a true objective function evaluation is made for the centroid of individuals selected, referred to as the centroid $x_{\text{centroid}}$. Offspring generation is the same as is discussed in $(\mu/\mu,\lambda)$-ES. The major difference  between $(\mu/\mu,\lambda)$-ES on noisy sphere and $(\mu/\mu,\lambda)$-ES with surrogate model assistance on sphere lies in the evaluation of selected individuals where the former uses a noise estimate and latter the surrogate model. The analysis is the same, therefore the normalized fitness advantage of an offspring $y$ over $x_{\text{centroid}})$ is 
...
\begin{center}
\begin{figure*}
\includegraphics[height=1.7in, width=6in]{success_NO_emergency_v3_final}
...
\begin{center}
\begin{figure*}
\includegraphics[height=1.7in, width=6in]{success_emergency_v3_final}

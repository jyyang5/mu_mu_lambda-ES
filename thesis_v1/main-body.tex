...
\end{align}
where $\vartheta = \sigma_\epsilon^*/\sigma^*$ is the noise-to-signal ratio, defined to measure the noise level relative to the algorithm's step size, the term $\sigma_{y,\epsilon} z_{y,\epsilon}$ denote the added noise. By changing of variable, $\sigma_{y,\epsilon} z_{y,\epsilon}$ can be viewed as a standard normally distributed term $\sigma_\epsilon z_\epsilon$. By adding the noise term to Eq. (\ref{eqn:delta}), we get the simplified Eq. (\ref{eqn:delta_noise}).
...
A GP is constructed using a set of evaluated points from random sampling in previous iterations, a Covariance Matrix Adaptation Evolution Strategy (CMA-ES) is used for parameter search in order to find the minimum of the GP prediction. To avoid false optima, a merit function is used to help explore new regions of the decision space, regarded as the fitness function for the CMA-ES.   
Predicted standard deviation  is used as the merit function that follows 
\begin{align}
f_M (x) = \hat f(x) - \alpha \sigma_{\text{GP}} (x),
\end{align}
where $\hat f(x)$ and $\sigma_{\text{GP}} (x)=\sqrt{\sum_*}$ (considered a single input $x \prime$ in Eq. (\ref{eqn:GP_cov})) are the GP estimate and standard deviation for data $x$, $\alpha \geq 0$ balances the two terms via scaling the density measure where a larger $\alpha$ pushes the search harder into unexplored region. 4 merit functions with $\alpha =0,1,2,4$ are used and being optimized in the context. Finally, the resulting minimum is added to the training set.

...
The objective function value on noisy sphere can be modeled by adding a Gaussian random variable with mean equals the true objective function value and some standard deviation referred to as noise strength $\sigma_\epsilon$. The noisy estimate of a candidate solution $x$ follows $f_{\epsilon}(x) = f(x) + \sigma_{\epsilon}z_\epsilon$ where $z_\epsilon \in \mathbb{R}$ is a standard normally distributed random variable that randomize the noise generated. By further introduces $\sigma_\epsilon^* = N \sigma_\epsilon / (2R^2)$, the normalized fitness noise \cite{1284729} and replace the accurate objective function evaluation with the noisy estimate, the normalized fitness advantage of $y$ on noisy sphere when $n \rightarrow \infty$ in Eq. (\ref{eqn:delta}) is
\begin{align}
...
\paragraph{Gaussin Process (GP)}

The GP is defined using the notation of $\color{red}{\text{slides by Arnold }}$: let $f(x)$ be an unkown scalar functuoon and $x \in \mathbb{R^N}$ is a point in an $N-$dimensional space. Evaluate $f$ at $K$ data points $X=(x_1,x_2,...,x_n)$ yields function values $f = (f(x_1),f(x_2),...,f(x_n))$. We want to predict new function values $f(X_*)$ of a test set $X_*$ with size $n_*$.

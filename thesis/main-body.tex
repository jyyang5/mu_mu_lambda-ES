...
% Figure 2 shows the relation of optimal expected fitness gain over noise-to-signal ratio derived in the limit of $n \rightarrow \infty$ for three different population sizes, followed by the coresponding optimial normalized step size over noise-to-signal ratio derived $n \rightarrow \infty$. and measured experimentally for $n \in \{10,100 \} $


% \begin{align}\label{no_noise_opt}
% \sigma^*_{opt} &= \ \mu c_{\mu / \mu, \lambda}\\
% \eta_{opt} &= \sigma^*_{opt} c_{\mu / \mu, \lambda} - \frac{(\sigma^*_{opt})^2}{2 \mu} \\
% & = \mu (c_{\mu / \mu, \lambda})^2- (\mu c_{\mu / \mu, \lambda})^2/2\mu \\
% & = \mu (c_{\mu / \mu, \lambda})^2/2
% \end{align}
...
\begin{center}
\begin{figure*}
\includegraphics[height=4in, width=6in]{merged_plot_v2}
\caption{Top row: Histogram showing the number of objective function calls needed to solve the five test problems. Second row: Convergence graphs for median runs. Third row: Relative model error obatined in median runs. Last row: normalized step size measured in median runs. }
...
\caption{The first two rows show the normalized convergence rate for each run plotted in histogram and normalized probability density function (pdf) respectively. The last two rows represent the success rate (proportion of good step size in each run) plotted in histogram and pdf respectively.}
...
The histogram showing the number of objective function calls needed to solve the test problems within the required accurancy are represented in the first row in \ref{opt_merged_plot}, the median objective funcation calls for each test problem is shown in \ref{Tab:Test_result}. The result of Arash and Dirk \cite{DBLP:conf/ppsn/KayhaniA18} using surrogate assisted (1+1)-ES is also included for comparision. The results of surrogate assisted $(\mu/\mu,\lambda)$-ES do not match the performance of surrogate assisted (1+1)-ES. Despite achieving a speed up from 1.2 to 1.9 for quartic function, the surrogate assisted $(\mu/\mu,\lambda)$-ES performs worse on sphere functions and even does not convergece in Schwefel\' s function. There is a trend in quadratic sphere and quartic function that the performance improves with a growing parental population, the number of objective function evaluations needed to solve linear sphere even increass after $\lambda>20$. There are a few reasons that may explain the big gap between experimental result and analytical result in Section 3, and the relation between the expected fitness gain and parental population size. Firstly, the step size is not appropriately adapted using CSA. Secondly the 
1. the step size is not appropriately adapted, 2. the  

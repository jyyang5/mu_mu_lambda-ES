...
\end{align}
where $E\| N(0,I) \|$ is the expected length of the search path $p$ and can be approximated as $E\| N(0,I) \| \approx \sqrt{n} (1-1/4n + 1/21n^2)$. In Section 4, we use the well established parameters from Hansen's CMA tutorial \cite{hansen2016cma} where $c = (\mu+2)/(N+\mu+5)$, $d=1+2 \max\left (0, \sqrt{(\mu-1)/(N+1)-1} \right)+c$.

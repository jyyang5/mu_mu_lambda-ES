...
\begin{align}
p_{k+1} \leftarrow (1-c)s_k + \sqrt{\mu c (2-c)} z
\end{align}

The step size is adapated 
\begin{align}
\sigma = \sigma \exp \left (  \frac{c}{d}  \left( \frac{\Vert p\Vert}{E \Vert N(O,I)\Vert } \right) \right )
\end{align}


refers to the consective steps the strategy taken, the strategy obatins information from previous steps and its corsponding mutations   

($\color{red}{not\ step\ size\ adaptation\ COULD\ IN\ FUTURE\ WORK}$) Covariance Matrix Adaptation (CMA) \cite{Hansen:2003:RTC:772374.772376}. The former uses an evolution path and a trial path 
...
It can be inferred from Figure 1 that, for a fixed population size, the expected fitness gain decreases with an increasing noise-to-signal-ratio. The surrogate cannot severe its purpose and the performance without 
...
\begin{center}
\begin{figure*}
\includegraphics[height=2.2in, width=5.8in]{opt_stepSize_fitGain_v1}

...
% By taking derivative we can find the opt.expected progress rate is $\eta_{opt} =  \sigma^*_{opt} c_{\mu \ \mu, \lambda} / \sqrt {1+ \vartheta^2} -  (\sigma^*_{opt})^2 /(2 \mu)$ and the coresponding opt. normalized step size is $\sigma^*_{opt} = \mu c_{\mu \ \mu, \lambda} /  \sqrt {1+ \vartheta^2}$.
% By taking derivative we can find the opt.expected progress rate is $\eta_{opt} = \frac{\sigma^*_{opt} c_{\mu \ \mu, \lambda}}{\sqrt {1+ \vartheta^2}} - \frac{(\sigma^*_{opt})^2}{2 \mu}$ and the coresponding opt. normalized step size is $\sigma^*_{opt} = \frac{ \mu c_{\mu \ \mu, \lambda}}{\sqrt {1+ \vartheta^2}} $.


% With the assumption that the cost of objective function calls are dominant in compotational cost in the search, the strategy converges proportionally to the negative slope of the objective function value over objective function values. So that the normalized convergece rate  
...
% Problem with equation labeling 
% \[\begin{equation}
% \begin{split}
% \Delta &= -\frac{N}{2} E \left [  \log f(y) - \log {f(x)} \right ] \\
%  &= -\frac{N}{2} E \left [  \log \frac{f(x^{t+1})}{f(x^{t})} \right ], \qedhere
%  \end{split}
% \end{euqation}\]



The expected fitness gian of against the normalized step size is plotted in Fig. 1. The line shows the result obtained from equation (xx) (xx) (xx). The dots represent the experimental result for unbiased Gaussin surroagte error for $n \in \{10,100 \}$ obtained by averaging 100 runs.

...
%
function oneLambda_RunALL(subplotNum)
...
FIG_NUM = 5;                
fun_precise_optFitGain_over_v(f,NUM_OF_RUNS,mu,lambda,v_array,10,opt_plot_colour,'x',FIG_NUM,c_mu_lambda,v_expedted_curve_array);    % plot n = 10 (x)
disp('* done'); 
fun_precise_optFitGain_over_v(f,NUM_OF_RUNS,mu,lambda,v_array,100,opt_plot_colour,'o',FIG_NUM,c_mu_lambda,v_expedted_curve_array);   % plot n = 100 (o)
disp('s done');
xlim([0 10]);
ylim([0 inf]);
...
title(p1,'fontsize',20);
if(subplotNum==3)
    legend('(3/3,10)-ES','(5/5,20)-ES','(10/10,40)-ES');
    box on;
    p2 = sprintf('opt_stepSize.fig');
    saveas(gcf,p2);
end

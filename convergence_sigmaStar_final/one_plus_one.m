...
function val = one_plus_one(f,x0,sigma_star,TRAINING_SIZE,NUM_OF_ITERATIONS,)%, OPTIMAL, TARGET_DISTANCE)
% initialization
...
%     GP_error(1:length(t_start+2:T)) = abs(fep_x(t_start+2:T)-f_x(t_start+2:T))./abs(f_x(t_start+1:T-1)-f_x(t_start+2:T));
    ...
    val = {t, x_array(:,T), fx, sigma_funEva_array, T, f_x, convergence_rate};

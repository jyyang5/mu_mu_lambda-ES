function val = mml_noise_revised(f,x0,sigma_star,sigma_ep_star,lambda,NUM_OF_ITERATIONS)
% initialization
...
while((t < NUM_OF_ITERATIONS) && f_centroid > 10^(-8))
    ...
    % selection
    [index, sorted_order] = sort(fy);         % sort fy/fyep (smaller first)
    y = y(:,sorted_order);
...
val = {t,centroid,f_centroid,sigma_array, centroid_array, fcentroid_array,convergence_rate};
